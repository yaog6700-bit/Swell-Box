...
func (t *winTray) showMenu() error {
	// Position like modern tray apps (WinUI flyouts): menu sits *above* the
	// taskbar / tray icon and does not cover the icon itself.
	// TPM_BOTTOMALIGN = bottom edge of menu at the anchor point → grows upward.
	const (
	...
	x, y := t.menuAnchorPoint()
	pSetForegroundWindow.Call(uintptr(t.window))

	// Center horizontally on the tray icon; bottom of menu at icon top / work area.
	flags := uintptr(TPM_BOTTOMALIGN | TPM_CENTERALIGN | TPM_VERTICAL | TPM_RIGHTBUTTON)
	...
	// Required so the menu dismisses correctly on the next click outside.
	pPostMessage.Call(uintptr(t.window), 0, 0, 0) // WM_NULL
	return nil
}

// menuAnchorPoint returns screen coords for TrackPopupMenu so the menu opens
// above the tray icon (or cursor as fallback), similar to WinUI3 tray flyouts.
func (t *winTray) menuAnchorPoint() (x, y int32) {
	// 1) Prefer the actual tray icon rectangle (Windows 7+).
	if r, ok := t.iconScreenRect(); ok {
		x = (r.Left + r.Right) / 2
		// Anchor just above the icon so the menu bottom does not cover it.
		y = r.Top
		if y < 1 {
			y = 1
		}
	...
	// 2) Cursor position (legacy path).
	p := point{}
	if res, _, _ := pGetCursorPos.Call(uintptr(unsafe.Pointer(&p))); res != 0 {
		x, y = p.X, p.Y
	}

	// 3) Nudge into the work area so tall menus stay above the taskbar strip.
	var wa struct{ Left, Top, Right, Bottom int32 }
	const spiGetWorkArea = 0x0030
	u32.NewProc("SystemParametersInfoW").Call(spiGetWorkArea, 0, uintptr(unsafe.Pointer(&wa)), 0)
	if wa.Bottom > wa.Top {
		if y > wa.Bottom {
			y = wa.Bottom
		}
		// Keep a few pixels above the taskbar edge when possible.
		if y > wa.Bottom-4 {
			y = wa.Bottom - 4
		}
	}
...
// iconScreenRect uses Shell_NotifyIconGetRect for the tray icon bounds.
func (t *winTray) iconScreenRect() (iconRect, bool) {
	t.muNID.Lock()
	nid := t.nid
	t.muNID.Unlock()
	if nid == nil || pShellNotifyIconGetRect.Find() != nil {
		return iconRect{}, false
	}
	...
	var rc iconRect
	r, _, _ := pShellNotifyIconGetRect.Call(
